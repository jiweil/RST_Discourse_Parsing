function[Trees]=ReadTrain(filename,isTraining)
    addpath('../multi');
    filename='train_tree.txt';
    isTraining=0;
    ...
    while ischar(tline)
        ...
        tree.Relation=100*ones(length(tree.LeafNodes),length(tree.LeafNodes));
        GetSpan(tree,tree.root);
        TrainSpan{i}=tree.Span;
        TrainRelation{i}=tree.Relation;
...
function[span]=GetSpan(tree,node)
    if node.isleaf==1
        ...
        tree.Relation(node.leaf_index,node.leaf_index)=node.relation;
    else
        for i=1:length(node.children)
        ...
        if(node.nuclear~=1) tree.Relation(left,right)=node.relation;
        end
    end
end
...
function[vector]=GetPositive(node,vector,parameter)
    if node.isleaf==1
        return;
    end
    for i=1:length(node.children)
        for j=i+1:length(node.children)
            if parameter.RelationToIndex(node.children{i}.relation,node.children{j}.relation)~=0
                vector=[vector;[node.children{i}.index,node.children{j}.index,parameter.RelationToIndex(node.children{i}.relation,node.children{j}.relation)]];
            end
        end
    end
    for i=1:length(node.children)
        vector=GetPositive(node.children{i},vector,parameter);
    end
end
